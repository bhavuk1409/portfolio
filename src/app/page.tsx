...
//
// TECHNICAL SKILLS — FIXED (NO GETTER)
//
...
// build the merged array safely
technicalSkills.all = [
  ...technicalSkills.languages,
  ...technicalSkills.ml_ai,
  ...technicalSkills.web_backend,
  ...technicalSkills.problem_solving,
];
    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-sm">
        <div className="container mx-auto flex h-16 max-w-6xl items-center justify-between px-4 md:px-6">
          <a href="#" className="text-2xl font-bold text-primary"></a>
      ...
      <main className="container mx-auto max-w-6xl flex-1 px-4 py-12 md:px-6 md:py-24">
        <section
          id="hero"
          className="grid grid-cols-1 gap-12 md:grid-cols-2 items-center min-h-[calc(100vh-12rem)]"
        >
          <div className="space-y-6">
            <p className="text-2xl">Hello,</p>
            <h1 className="text-4xl font-bold tracking-tight sm:text-5xl md:text-6xl">
              This is <span className="text-primary">{personalInfo.name}</span>, I'm Passionate About
              AI, and Solving Real-World Problems.
            </h1>
            ...
            <div className="flex gap-4">
              ...
              <Button asChild variant="secondary" size="lg">
                <a href="/BHAVUK_AG_RESUME_UPDATED.pdf" download>
                  Get Resume
                </a>
              </Button>
            </div>
          ...
          <div>
            <AnimatedCode name={personalInfo.name} skills={technicalSkills.all} />
          </div>
        </section>
        ...
        <div className="space-y-24">
          ...
          <AnimatedSection id="about" className="mt-20">
            <h2 className="mb-8 text-3xl font-bold tracking-tight text-primary">About Me</h2>
            <p className="text-lg leading-relaxed text-foreground/80">{personalInfo.bio}</p>
          </AnimatedSection>
          ...
          <AnimatedSection id="experience">
            ...
            <div className="space-y-8 relative border-l-2 border-primary/30">
              {workExperience.map((exp, index) => (
                <Card
                  key={index}
                  className="relative pl-8 md:pl-12 bg-transparent border-0 shadow-none before:content-[''] before:absolute before:left-[-9px] before:top-1 before:w-4 before:h-4 before:rounded-full before:bg-primary"
                >
                  <CardHeader className="p-0">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center">
                      <CardTitle className="text-2xl">{exp.role}</CardTitle>
                      <Badge variant="outline" className="mt-2 sm:mt-0 border-primary/50 text-primary">
                        {exp.duration}
                      </Badge>
                    </div>
            ...
            <Card className="bg-card/50">
              <CardContent className="p-6 space-y-6">
                ...
                <SkillCategory
                  ...
                  skills={technicalSkills.problem_solving}
                />
              </CardContent>
            </Card>
          </AnimatedSection>
